...
interface SpeechInteractionProps {
  object: ObjectData;
}

const SpeechInteraction: React.FC<SpeechInteractionProps> = ({ object }) => {
  ...
  const novaSonicClientRef = useRef<NovaSonicClient | null>(null);
  ...
  useEffect(() => {
    ...
    client.onTextOutput((data: TextOutput) => {
    ...
    client.onAudioOutput((audioData: Float32Array) => {
  ...
  const startListening = async () => {
    try {
      ...
      await client.startListening(object.prompt || null);
