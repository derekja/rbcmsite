...
// Only using the Sheets API base in this file
...
class DataService {
  ...
  async checkForImageUpdates(): Promise<void> {
    try {
      ...
      try {
        ...
        console.warn('Could not parse manifest JSON:', parseError instanceof Error ? parseError.message : String(parseError));
      }
    } catch (error) {
      // Just log the error but don't disrupt the app flow
      console.warn('Could not check for image updates:', error instanceof Error ? error.message : String(error));
    }
  }
  ...
  updateObjectPrompt(objectId: string | number, newPrompt: string): boolean {
  ...
  getObjectById(objectId: string | number): ObjectData | undefined {
  ...
  async fetchPrompts(): Promise<PromptData[]> {
    try {
      ...
      return values.slice(startRow)
        .filter((row: any[]) => row && row.length >= 2 && row[0] && row[1]) // Ensure row has both name and prompt
  ...
  async getObjectsData(): Promise<ObjectData[]> {
    try {
