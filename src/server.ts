...
const clientModulePath = path.join(__dirname, 'components', 'nova-sonic');
...
try {
  ...
  bedrockClient = new NovaSonicBidirectionalStreamClient({
    requestHandlerConfig: {
      ...
      // connectionTimeout is not in NodeHttp2HandlerOptions type
    },
  ...
  console.log('Available methods on bedrockClient:', 
    Object.getOwnPropertyNames(Object.getPrototypeOf(bedrockClient))
      .filter(method => typeof (bedrockClient as any)[method] === 'function')
  );
} catch (error) {
...
// Track all active sessions globally for debugging
const globalActiveSessions = new Map<string, { createdAt: Date, socketId: string }>();
...
io.engine.on('connection', (rawSocket) => {
  ...
  rawSocket.on('upgrading', (transport: any) => {
...
io.on('connection', (socket) => {
  ...
  socket.on = function(event: string, listener: any) {
    ...
    return originalOn.call(this, event, function(this: any, ...args: any[]) {
      console.log(`Received event ${event} with args:`, args.slice(0, 1));
  ...
  globalActiveSessions.set(sessionId, { createdAt: new Date(), socketId: socket.id });
  console.log(`Active sessions: ${globalActiveSessions.size}`);
  globalActiveSessions.forEach((session, id) => {
  ...
  try {
    ...
    // Initiate the session immediately
    try {
      bedrockClient.initiateSession(sessionId);
      console.log(`Session bidirectional stream initiated for: ${sessionId}`);
    } catch (err) {
      console.error(`Error initiating session: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    socket.on('initSession', async (data: { prompt?: string }, acknowledge?: (data: any) => void) => {
      try {
        ...
        if (bedrockClient.isSessionActive(sessionId)) {
          try {
            ...
            try { await streamSession.endAudioContent(); } catch (e) { console.log('No audio to close'); }
            try { await streamSession.endPrompt(); } catch (e) { console.log('No prompt to close'); }
            try { await streamSession.close(); } catch (e) { console.log('Error closing session:', e instanceof Error ? e.message : String(e)); }
          } catch (cleanErr) {
            console.warn(`Error during clean session close: ${cleanErr instanceof Error ? cleanErr.message : String(cleanErr)}`);
          }
          ...
          try {
            ...
            console.warn(`Error force closing session: ${forceErr instanceof Error ? forceErr.message : String(forceErr)}`);
          }
        }
        ...
        try {
          ...
          socket.emit('error', { 
            ...
            details: `Session ${sessionId} could not be created: ${recErr instanceof Error ? recErr.message : String(recErr)}` 
          });
          return;
        }
        ...
        try {
          ...
          console.error('Error details:', {
            ...
            code: err instanceof Error ? (err as any).code : 'No code'
          });
          socket.emit('error', {
            message: 'Failed to create bidirectional stream',
    ...
    socket.on('audioStart', async (data: any) => {
      try {
